...
当前 Android Demo 已提供完整 P0 闭环：选择场景、进入横屏练习页、语音或演示输入、云端教练优先分析、本地 fallback、TTS 朗读、评分纠错和课后总结。设置页可切换 USB 真机、模拟器或自定义 FastAPI 地址，也可选择稳定演示、效果优先、离线优先三种引擎策略；练习页会展示当前 ASR、TTS、判定链路，离线优先会直接使用本地分析，效果优先在预留云端增强入口的同时保留本地兜底。
